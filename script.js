...
function startTimer() {
  timerInterval = setInterval(() => {
    time++;
    timerEl.textContent = `진행 시간: ${time}초`;
  }, 1000);
}

function showQuestion() {
  const quiz = quizData[current];
  questionText.textContent = `${current + 1}. ${quiz.question}`;
...
checkBtn.onclick = () => {
  ...
  if (quiz.type === "multiple") {
    ...
    answerEl.innerHTML = `정답: <span class="correct-answer">${
      Array.isArray(quiz.answer) ? quiz.answer.join(", ") : quiz.answer
    }</span>`;
    choicesBox.appendChild(answerEl);
  }
  ...
  choicesBox.appendChild(explainEl);
...
nextBtn.onclick = () => {
  ...
  if (quiz.type === "subjective") {
    ...
    } while (quizData[current].type !== "subjective");
    showQuestion();
  } else {
    current++;
    if (correctCount >= 4) {
      while (
        ...
        quizData[current].type === "multiple"
      ) {
        current++;
      }
    }
